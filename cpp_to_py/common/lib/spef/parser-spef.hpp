...
inline void validate_token_spef(std::string& name) {
    // remove '\'
...
inline void Spef::expand_name(Net& net){
  ...
  validate_token_spef(net.name);
  for(auto &c : net.connections){
    expand_string(c.name, name_map);
    expand_string(c.driving_cell, name_map);
    validate_token_spef(c.name); 
    validate_token_spef(c.driving_cell);
  }

  for(auto &t: net.caps){
    expand_string(std::get<0>(t), name_map);
    expand_string(std::get<1>(t), name_map);
    validate_token_spef(std::get<0>(t));
    validate_token_spef(std::get<1>(t));
  }

  for(auto &r: net.ress){
    expand_string(std::get<0>(r), name_map);
    expand_string(std::get<1>(r), name_map);
    validate_token_spef(std::get<0>(r));
    validate_token_spef(std::get<1>(r));
  }
}
