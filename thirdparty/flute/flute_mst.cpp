...
#define D2M FLUTE_D2(1)     // Max net degree that flute_mr will handle
#define MR_FOR_SMALL_CASES_ONLY 1
#if MR_FOR_SMALL_CASES_ONLY
#define MAXPART D2M    // max partition of an MST
#define MAXT (D2M/D*2)
#else
#define MAXPART (d/9*2) //(FLUTE_MAXD/THD*2) // max partition of an MST
...
#define MAX_HEAP_SIZE (FLUTE_MAXD*2)
DTYPE **hdist;
typedef struct node_pair_s { // pair of nodes representing an edge
  int node1, node2;
} node_pair;
node_pair *heap; //heap[FLUTE_MAXD*FLUTE_MAXD];
int heap_size=0;
	      ...
	      int **nb)
{
  ...
  for (i=0; i<MAXT; i++) {
    //order[i] = &(order_base[i*FLUTE_D2(acc)]);
	      ...
	      DTYPE *threshold_x, DTYPE *threshold_y, DTYPE *threshold)
{
  ...
  int prev_effort;
  /*
  int num_subtree, subroot[MAXPART], suproot[MAXPART], isSuperRoot[FLUTE_MAXD];
  int tree_id[FLUTE_MAXD], tid, tree_size[FLUTE_MAXD], edges[2*FLUTE_MAXD];
  int idx[MAXPART], offset[MAXPART], *order[MAXT],
        order_base[FLUTE_MAXD+10]; //order_base[MAXT*FLUTE_MAXD];
  DTYPE x[FLUTE_MAXD+MAXPART], y[FLUTE_MAXD+MAXPART];
  int new_s[FLUTE_MAXD+MAXPART], si[FLUTE_MAXD], xmap[FLUTE_MAXD+MAXPART];
  */
  DTYPE *x, *y;
  ...
  /*
  for (i=0; i<MAXT; i++) {
    order[i] = &(order_base[i*FLUTE_MAXD]);
  }
  */
...
Tree flutes_HD(int d, DTYPE *xs, DTYPE *ys, int *s, int acc)
{
  int i, A, orig_D3;
  Tree t;
  //DTYPE *dist[FLUTE_MAXD], *dist_base;
